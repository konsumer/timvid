...
void zxspectrum_clash(out vec4 fragColor, in vec4 fragCoord) {
  vec2 pv = floor(fragCoord.xy / LOWREZ);
  vec2 bv = floor(pv / 8.0) * 8.0;
  vec2 sv = floor(u_resolution / LOWREZ);
  ...
  for (int py = 1; py < 8; py++) {
    for (int px = 0; px < 8; px++) {
      vec4 cs = bmap((texture2D(u_tex0, (bv + vec2(px, py)) / sv).rgb));
      bright += cs.a;
  ...
  vec3 cs = texture2D(u_tex0, pv / sv).rgb;
...
void zxspectrum_colors(out vec4 fragColor, in vec4 fragCoord) {
  vec2 pv = floor(fragCoord.xy / LOWREZ);
  vec2 sv = floor(u_resolution / LOWREZ);

  vec4 cs = smap(texture2D(u_tex0, pv / sv).rgb);
...
void main(void) {
  // zxspectrum_colors(gl_FragColor, gl_FragCoord);
  zxspectrum_clash(gl_FragColor, gl_FragCoord);
}
